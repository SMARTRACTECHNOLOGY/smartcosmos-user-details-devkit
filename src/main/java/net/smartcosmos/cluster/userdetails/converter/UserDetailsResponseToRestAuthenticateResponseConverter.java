...
public class UserDetailsResponseToRestAuthenticateResponseConverter
    ...
    public RestAuthenticateResponse convert(UserDetailsResponse source) {
        ...
        return RestAuthenticateResponse.builder().userUrn(source.getUrn()).username(source.getUsername()).passwordHash(source.getPasswordHash())
                .tenantUrn(source.getTenantUrn()).authorities(source.getAuthorities()).build();
    }
